...
/* The SIZE distribution of those sections (issue #236), which is a different
 * question on the same walk and is why it rides this census instead of a
 * second one.
 *
 * #236's lever is the lane->stream mapping and the staging of decoded literals
 * - write straight to the destination, or stage in shared memory and flush
 * coalesced. What bounds it is how much literal work one section hands the
 * warp in one go, and that is a property of what the compressor emits rather
 * than of any kernel, so it is measurable now. #236 says so in its own body:
 * the literal-section size distribution is the durable result whichever way
 * the A/B later goes.
 *
 * IT IS ALSO THE ONE STATISTIC THAT SAYS WHETHER AN M1 RESULT TRANSFERS.
 * docs/BENCHMARKS.md "Perf pass 1 (issue #16)" rejected the vectorized 16-byte
 * literal copy at -6% because Silesia's LZ4 literal runs are mostly shorter
 * than 16 bytes, so the wide path rarely triggered. That is a statement about
 * a per-sequence literal RUN. A Zstd literals section is a whole block's
 * literals decoded in bulk, a different population entirely, and the only
 * honest way to say whether the negative carries over is to measure the
 * population it would carry over to.
 *
 * WHY bench/literal_hist.h IS NOT REUSED. That header exists so LZ4's and
 * Snappy's literal-run histograms share one set of bucket edges, because the
 * question there is a comparison between two harnesses over the same bytes.
 * Its edges sit around 16 for the wide-copy path and its walk drives a shipped
 * parser element by element. Neither the subject nor the scale is this one -
 * a section reaches 262143 bytes where a run rarely reaches 256 - so reusing
 * it would put two unrelated populations behind one set of edges, which is the
 * failure that header was written to prevent rather than an instance of it.
 * The lesson it carries IS reused: both statistics, always, section share
 * beside byte share.
 * ---------------------------------------------------------------------- */

constexpr unsigned kLitSectionBuckets = 6;

/* Inclusive upper edges in regenerated bytes, and the ONLY place they are
 * written: the bucket function scans this array and the labels are printed
 * from it, so a bucket cannot end up named one thing and filled by another.
 *
 * Two of the edges are the format's and are not adjustable taste:
 *   127/128    a warp's coalesced store is 32 lanes x 4 bytes = 128 bytes, so
 *              a section below it cannot fill one transaction and no staging
 *              shape can buy a coalesced flush out of it.
 *   1023/1024  the Size_Format=00 regenerated ceiling (RFC 8878 3.1.1.3.1.1),
 *              which is the boundary the census above is about, so the two
 *              reports share an edge and can be read against each other.
 * The three between them are powers of two and carry no claim; they are there
 * so the shape of the tail is readable rather than to mark anything. The last
 * bucket is open because the wide spellings reach 262143. */
constexpr size_t kLitSectionUpper[kLitSectionBuckets] = {
    127, 1023, 4095, 16383, 65535, static_cast<size_t>(-1)};

/* The first bucket at or above the coalescing floor, named once so the
 * derived trigger line below cannot drift away from the edge it is about. */
constexpr unsigned kLitSectionCoalesceBucket = 1;
static_assert(kLitSectionUpper[kLitSectionCoalesceBucket - 1] == 127,
              "the coalescing-floor trigger line names 128 bytes; the edge "
              "table under it must still put 127 at the top of the bucket "
              "below, or the line reports a threshold nobody chose");

/* Inclusive lower edge of a bucket, derived rather than tabulated. */
constexpr size_t LitSectionLower(unsigned bucket) {
    return bucket == 0 ? 0 : kLitSectionUpper[bucket - 1] + 1;
}

unsigned LitSectionBucket(size_t regenerated) {
    for (unsigned b = 0; b + 1 < kLitSectionBuckets; b++) {
        if (regenerated <= kLitSectionUpper[b]) {
            return b;
        }
    }
    return kLitSectionBuckets - 1;
}
...
struct LiteralsCensus {
    ...
    /* The size distribution over those same huffman sections - the population
     * the staging lever touches, and the one #236 asks to have recorded.
     * smallest/largest are the sizes actually placed in each bucket and are
     * meaningful only where bucket_sections is non-zero; they exist so a
     * misrouted section is caught individually rather than averaged away. */
    size_t bucket_sections[kLitSectionBuckets] = {0};
    size_t bucket_bytes[kLitSectionBuckets] = {0};
    size_t bucket_smallest[kLitSectionBuckets] = {0};
    size_t bucket_largest[kLitSectionBuckets] = {0};
};
...
bool CensusOfFrame(const std::vector<unsigned char>& frame,
    ...
    for (const ZstdBlockShape& block : shape.blocks) {
        ...
        /* The same section folded into the size distribution. smallest is
         * seeded on the bucket's first section rather than from a sentinel,
         * which is why it is taken before the count is raised. */
        const size_t regenerated = block.literals_regenerated_size;
        const unsigned bucket = LitSectionBucket(regenerated);
        if (census->bucket_sections[bucket] == 0 ||
            regenerated < census->bucket_smallest[bucket]) {
            census->bucket_smallest[bucket] = regenerated;
        }
        if (regenerated > census->bucket_largest[bucket]) {
            census->bucket_largest[bucket] = regenerated;
        }
        census->bucket_sections[bucket]++;
        census->bucket_bytes[bucket] += regenerated;
    }
    return true;
}

/* THE DISTRIBUTION HAS TO ADD UP TO THE CENSUS IT RIDES ON, and this is what
 * says so on every run rather than only under --selfcheck.
 *
 * Three legs, each for a different way the bucketing can be wrong while the
 * table still prints:
 *   - the section count and the byte total have to equal the huffman totals
 *     the census already keeps, so a section dropped or counted twice is
 *     caught by arithmetic rather than by a reader noticing a row looks small;
 *   - every occupied bucket holds only sizes inside its own edges, checked
 *     against the smallest and largest actually placed there rather than
 *     against the bucket's average, because an average hides one misrouted
 *     section among hundreds;
 *   - a bucket that counted sections carrying bytes and folded none of them
 *     into its byte total is a half-applied fold rather than a corpus that
 *     happens to be empty. Only that direction is checked here: bytes with no
 *     sections cannot arise, because the two are raised together, and dropping
 *     the section side instead moves the totals leg above.
 *
 * A dead read of Regenerated_Size is NOT this function's job and is already
 * refused one guard over: LiteralsCensusSeesBothSpellings requires both
 * spellings counted in sections AND in bytes over the forced-mode corpus, and
 * a Regenerated_Size stuck at zero fails it there. Repeating that here would
 * be a second copy of a refusal, not a second refusal. */
bool LiteralsDistributionConsistent(const LiteralsCensus& census,
                                    std::string* why) {
    const size_t want_sections =
    ...
    size_t got_sections = 0;
    size_t got_bytes = 0;
    for (unsigned b = 0; b < kLitSectionBuckets; b++) {
        got_sections += census.bucket_sections[b];
        got_bytes += census.bucket_bytes[b];
        if (census.bucket_sections[b] == 0) {
            continue;
        }
        if (census.bucket_smallest[b] < LitSectionLower(b) ||
            census.bucket_largest[b] > kLitSectionUpper[b]) {
            *why = "a literals-size bucket holds a section outside its own "
                   "edges, so the row is labelled one range and filled from "
                   "another";
            return false;
        }
        if (census.bucket_bytes[b] == 0 && census.bucket_largest[b] != 0) {
            *why = "a literals-size bucket counted sections carrying bytes "
                   "but folded none of them into its byte total";
            return false;
        }
    }
    if (got_sections != want_sections || got_bytes != want_bytes) {
        *why = "the literals-size distribution does not add up to the huffman "
               "totals it was folded from";
        return false;
    }
    return true;
}
...
void PrintLiteralsReport(const ZstdCorpus& corpus,
    ...
    /* The size distribution, both statistics on every row for the reason the
     * spelling census above prints both: how OFTEN a size occurs and how much
     * of the work sits at that size are different questions, and only the
     * second one bounds what a staging shape could buy. */
    size_t floor_sections = 0;
    size_t floor_bytes = 0;
    for (unsigned b = kLitSectionCoalesceBucket; b < kLitSectionBuckets; b++) {
        floor_sections += census.bucket_sections[b];
        floor_bytes += census.bucket_bytes[b];
    }
    for (unsigned b = 0; b < kLitSectionBuckets; b++) {
        char label[40];
        if (b + 1 == kLitSectionBuckets) {
            std::snprintf(label, sizeof(label), "%zu+", LitSectionLower(b));
        } else {
            std::snprintf(label, sizeof(label), "%zu-%zu", LitSectionLower(b),
                          kLitSectionUpper[b]);
        }
        std::printf("- huffman sections sized %-13s: %8zu sections (%8.4f%%), "
                    "%13zu bytes (%8.4f%%)\n",
                    label, census.bucket_sections[b],
                    SharePercent(census.bucket_sections[b], huffman_sections),
                    census.bucket_bytes[b],
                    SharePercent(census.bucket_bytes[b], huffman_bytes));
    }
    std::printf("- at or above the 128-byte coalescing floor (32 lanes x 4 "
                "bytes, the smallest store a warp can make whole): %.4f%% of "
                "huffman sections, %.4f%% of huffman literal bytes\n",
                SharePercent(floor_sections, huffman_sections),
                SharePercent(floor_bytes, huffman_bytes));
    std::printf("- mean huffman section: %.1f bytes regenerated over %zu "
                "sections\n",
                huffman_sections == 0
                    ? 0.0
                    : static_cast<double>(huffman_bytes) /
                          static_cast<double>(huffman_sections),
                huffman_sections);
}
...
bool RunLiteralsCensus(const std::vector<unsigned char>& source,
    ...
    std::string why;
    if (!LiteralsDistributionConsistent(census, &why)) {
        std::fprintf(stderr, "the literals-size distribution over %s is not "
                             "self-consistent: %s\n",
                     corpus.name.c_str(), why.c_str());
        return false;
    }
...
/* THE EDGE WALK, and it is the near-miss guard rather than a restatement of
 * the table.
 *
 * The bucketing is one loop over one array, so a bucket cannot be labelled
 * from different numbers than it is filled from. What that construction does
 * NOT rule out is the one-character mistake inside the loop - `<` where `<=`
 * belongs, or a returned index one off - and every such mistake moves exactly
 * the sections sitting ON an edge, which is the population a corpus is least
 * likely to contain and a reader least likely to notice. So the edges are
 * walked directly instead of being waited for: each inclusive upper edge must
 * land in its own bucket and each edge plus one in the next.
 *
 * It is checked here rather than only over a corpus because a fixture of an
 * exactly-1023-byte literals section is not something the reference can be
 * asked for: Regenerated_Size is whatever the compressor decided to leave as
 * literals in a block, and no public entry point sets it. */
bool LitSectionBucketEdgesHold() {
    bool ok = true;
    if (LitSectionBucket(0) != 0) {
        std::fprintf(stderr, "an empty literals section does not land in the "
                             "first size bucket\n");
        ok = false;
    }
    for (unsigned b = 0; b < kLitSectionBuckets; b++) {
        if (LitSectionBucket(LitSectionLower(b)) != b) {
            std::fprintf(stderr,
                         "a section of %zu bytes does not land in the bucket "
                         "whose range opens at that size\n",
                         LitSectionLower(b));
            ok = false;
        }
        if (b + 1 == kLitSectionBuckets) {
        ...
        if (LitSectionBucket(kLitSectionUpper[b]) != b) {
            std::fprintf(stderr,
                         "a section of %zu bytes does not land in the bucket "
                         "whose range closes at that size\n",
                         kLitSectionUpper[b]);
            ok = false;
        }
        if (LitSectionBucket(kLitSectionUpper[b] + 1) != b + 1) {
            std::fprintf(stderr,
                         "a section one byte past the %zu-byte edge does not "
                         "land in the next bucket up\n",
                         kLitSectionUpper[b]);
            ok = false;
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (literals) {
        ...
        if (selfcheck && !LitSectionBucketEdgesHold()) {
            return 1;
        }
        if (selfcheck) {
